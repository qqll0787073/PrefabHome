...
test("restores portal workspace and selected logistics request from query state", () => {
  ...
  assert.equal(buildPortalSearch({ view: "dashboard", workspace: "logistics", requestId: "request-1" }), "?view=dashboard&workspace=logistics&request=request-1");
...
test("keeps request identifiers out of unrelated URLs", () => {
  assert.equal(buildPortalSearch({ view: "dashboard", workspace: "rfqs", requestId: "request-1" }), "?view=dashboard&workspace=rfqs");
  assert.equal(buildPortalSearch({ view: "browse", workspace: "logistics", requestId: "request-1" }), "");
