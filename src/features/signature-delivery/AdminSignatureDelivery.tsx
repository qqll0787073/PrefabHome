...
interface AdminSignatureDeliveryProps {
  authMode: "supabase" | "demo";
}

export function AdminSignatureDelivery({ authMode }: AdminSignatureDeliveryProps) {
    ...
    async function loadDeliveries() {
      ...
      try {
        if (authMode === "demo") {
          ...
          const deliveryRows = await fetchAdminSignatureDeliveries();
  ...
  return (
    <section className="quote-panel">
      <h4>Signature Delivery Management</h4>
      {isLoading && <LoadingState message="Loading signature delivery requests..." />}
      ...
      <p className="form-notice">Admin signature delivery access is read-only.</p>
    </section>
  );
}
