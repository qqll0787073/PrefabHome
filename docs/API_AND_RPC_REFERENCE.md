...
| Manufacturers, `src/lib/manufacturers.ts` | `fetchOwnManufacturerAccount`, `saveManufacturerApplication`, `fetchManufacturerApplications`, `reviewManufacturerApplication` | Auth-derived own-account requests and explicit normalized form/review values; returns safe account or application records | Manufacturer owns one draft/rejected application; Admin reviews all. Duplicate owner, locked status, validation, cross-owner, and role escalation are denied. |
