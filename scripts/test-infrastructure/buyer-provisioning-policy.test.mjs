...
test("Buyer provisioning introduces no migration or manual profile SQL", () => {
  assert.deepEqual(readdirSync("supabase/migrations").filter((item) => item.endsWith(".sql")).map((item) => item.slice(0, 4)), Array.from({ length: 31 }, (_, index) => String(index + 1).padStart(4, "0")));
