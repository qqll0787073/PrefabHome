...
test("Admin provisioning remains isolated with migrations 0001 through 0033", () => {
  const versions = readdirSync("supabase/migrations").filter((name) => /^\d{4}_.+\.sql$/.test(name)).map((name) => name.slice(0, 4)).sort();
  assert.deepEqual(versions, Array.from({ length: 33 }, (_, index) => String(index + 1).padStart(4, "0")));
});
