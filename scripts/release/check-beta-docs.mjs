...
export function auditBetaDocs(root = process.cwd()) {
  ...
  if (!existsSync(migrationDirectory)) {
    ...
    if (JSON.stringify(versions) !== JSON.stringify(EXPECTED_MIGRATIONS)) {
      errors.push(`Migration versions must be exactly 0001-0034; found ${versions.join(", ") || "none"}.`);
    }
  }
...
function main() {
  ...
  console.log(`Beta release document audit passed (${REQUIRED_BETA_FILES.length} required files, ${EXPECTED_MIGRATIONS.length} migrations).`);
}
