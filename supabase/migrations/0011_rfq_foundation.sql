...
grant select, delete on table public.rfq_events to authenticated;
revoke insert, update on table public.rfq_events from authenticated;
...
begin
  ...
  if event_name = 'buyer_opened' then
    if not exists (
      select 1 from public.rfqs
      where id = rfq_uuid
        and buyer_id = auth.uid()
    ) then
      raise exception 'Only the RFQ buyer can record buyer-opened events.';
    end if;
    ...
    if not exists (
      ...
      raise exception 'Only the assigned manufacturer can record manufacturer-opened events.';
    end if;
  else
    raise exception 'RFQ event type must be generated by a trusted lifecycle or message flow.';
  end if;

  perform public.insert_trusted_rfq_event(rfq_uuid, event_name, auth.uid(), event_metadata);
end;
$$;

create or replace function public.record_rfq_opened(
  rfq_uuid uuid
)
returns void
language plpgsql
...
begin
  ...
  select case
    when r.buyer_id = auth.uid() then 'buyer_opened'
    when public.owns_manufacturer(r.manufacturer_id) then 'manufacturer_opened'
    else null
  end
  into opened_event
  from public.rfqs r
...
revoke all on function public.record_rfq_event(uuid, text, jsonb) from public, anon;
grant execute on function public.record_rfq_event(uuid, text, jsonb) to authenticated;
...
begin
  ...
  if new.event_type in ('quote_created') then
    raise exception 'RFQ quote events are deferred to PH-006B trusted quote flow.';
  end if;
...
begin
  if tg_op = 'INSERT' then
    ...
    lifecycle_event := case new.status
      when 'submitted' then 'submitted'
      when 'manufacturer_review' then 'manufacturer_opened'
      when 'cancelled' then 'cancelled'
      when 'accepted' then 'accepted'
      when 'declined' then 'declined'
      when 'expired' then 'expired'
  ...
  if lifecycle_event is not null then
    perform public.insert_trusted_rfq_event(new.id, lifecycle_event, auth.uid(), '{}'::jsonb);
  end if;
...
drop trigger if exists record_rfq_lifecycle_event on public.rfqs;
create trigger record_rfq_lifecycle_event
after insert or update on public.rfqs
for each row execute function public.record_rfq_lifecycle_event();

drop trigger if exists record_rfq_message_event on public.rfq_messages;
create trigger record_rfq_message_event
after insert on public.rfq_messages
for each row execute function public.record_rfq_message_event();
...
create policy "rfqs_update_submitted_owned_manufacturer"
on public.rfqs
for update
to authenticated
using (
...
with check (
  public.owns_manufacturer(manufacturer_id)
  and status = 'manufacturer_review'
);
