...
create temp table signature_delivery_foundation_results (
  check_name text primary key,
  passed boolean not null,
  detail text not null default ''
) on commit drop;

grant all on table signature_delivery_foundation_results to authenticated, anon;

do $$
declare
  package_row public.signature_packages%rowtype;
  delivery_row public.signature_delivery_requests%rowtype;
  queued_row public.signature_delivery_requests%rowtype;
  cancelled_row public.signature_delivery_requests%rowtype;
  buyer_uuid uuid;
...
begin
  ...
  where sp.status = 'ready_to_send'
    and not exists (
      select 1
      from public.signature_delivery_requests sdr
      where sdr.signature_package_id = sp.id
    )
    and exists (
      select 1
      from public.signature_participants p
      where p.signature_package_id = sp.id
        and p.participant_role = 'buyer_signer'
        and p.signing_order = 1
    ...
    and exists (
      ...
      from public.signature_participants p
      where p.signature_package_id = sp.id
        and p.participant_role = 'manufacturer_signer'
        and p.signing_order = 2
        and p.full_name is not null
        and p.email is not null
    )
  ...
  if package_row.id is null then
    raise exception 'No ready_to_send Signature Package with complete participants is available for PH-008D rollback verification.';
  end if;

  buyer_uuid := package_row.buyer_id;
  select owner_id into manufacturer_owner_uuid from public.manufacturers where id = package_row.manufacturer_id;
  select id into admin_uuid from public.profiles where role = 'admin' order by created_at desc limit 1;

  perform set_config('request.jwt.claim.role', 'authenticated', true);
  perform set_config('request.jwt.claim.sub', buyer_uuid::text, true);
  set local role authenticated;

  select * into delivery_row from public.create_signature_delivery_request(package_row.id);
  insert into signature_delivery_foundation_results values ('Buyer creates delivery request from ready package', delivery_row.signature_package_id = package_row.id, delivery_row.delivery_number);
  insert into signature_delivery_foundation_results values ('delivery number database-generated', delivery_row.delivery_number is not null, delivery_row.delivery_number);
  insert into signature_delivery_foundation_results values ('valid SDL number format', delivery_row.delivery_number ~ '^SDL-[0-9]{4}-[0-9]{6}$', delivery_row.delivery_number);
  insert into signature_delivery_foundation_results values ('provider unconfigured', delivery_row.provider_key = 'unconfigured', delivery_row.provider_key);
  insert into signature_delivery_foundation_results values ('status starts delivery_draft', delivery_row.status = 'delivery_draft', delivery_row.status);
  insert into signature_delivery_foundation_results values ('source fields database-derived', delivery_row.package_number = package_row.package_number and delivery_row.contract_id = package_row.contract_id and delivery_row.contract_number = package_row.contract_number and delivery_row.buyer_id = package_row.buyer_id and delivery_row.manufacturer_id = package_row.manufacturer_id and delivery_row.created_by = buyer_uuid, 'source checked');
  insert into signature_delivery_foundation_results values ('package snapshot captured', delivery_row.package_snapshot->>'id' = package_row.id::text, delivery_row.package_snapshot->>'package_number');
  insert into signature_delivery_foundation_results values ('recipient snapshot captured', jsonb_typeof(delivery_row.recipient_snapshot) = 'array' and jsonb_array_length(delivery_row.recipient_snapshot) = 2, jsonb_array_length(delivery_row.recipient_snapshot)::text);
  insert into signature_delivery_foundation_results values ('payload snapshot internal only', delivery_row.request_payload_snapshot->>'provider_key' = 'unconfigured' and delivery_row.request_payload_snapshot->>'provider_contacted' = 'false' and delivery_row.request_payload_snapshot->>'email_sent' = 'false' and delivery_row.request_payload_snapshot->>'signing_link_created' = 'false' and delivery_row.request_payload_snapshot->>'contract_signed' = 'false', 'payload checked');

  select count(*) into recipient_count
  from public.signature_delivery_recipients
  where delivery_request_id = delivery_row.id;
  insert into signature_delivery_foundation_results values ('exactly two recipients derived', recipient_count = 2, recipient_count::text);
  insert into signature_delivery_foundation_results values (
    'fixed recipient order preserved',
    exists (select 1 from public.signature_delivery_recipients where delivery_request_id = delivery_row.id and participant_role = 'buyer_signer' and signing_order = 1)
      and exists (select 1 from public.signature_delivery_recipients where delivery_request_id = delivery_row.id and participant_role = 'manufacturer_signer' and signing_order = 2),
    'order checked'
  );
  insert into signature_delivery_foundation_results values (
    'recipient status pending only',
    not exists (select 1 from public.signature_delivery_recipients where delivery_request_id = delivery_row.id and delivery_status <> 'pending'),
    'pending checked'
  );

  blocked := false;
  begin
    perform public.create_signature_delivery_request(package_row.id);
  exception when others then
    blocked := true;
  end;
  insert into signature_delivery_foundation_results values ('duplicate delivery request denied', blocked, 'blocked: ' || blocked);
  ...
  insert into signature_delivery_foundation_results values ('Manufacturer creation denied', blocked, 'blocked: ' || blocked);
  ...
  insert into signature_delivery_foundation_results values ('Admin creation denied', blocked, 'blocked: ' || blocked);
  ...
  insert into signature_delivery_foundation_results values ('Anonymous creation denied', blocked, 'blocked: ' || blocked);
  reset role;
  set local role authenticated;

  perform set_config('request.jwt.claim.sub', manufacturer_owner_uuid::text, true);
  select count(*) into visible_count from public.signature_delivery_requests where id = delivery_row.id;
  insert into signature_delivery_foundation_results values ('assigned Manufacturer read', visible_count = 1, 'visible: ' || visible_count);

  blocked := false;
  begin
    perform public.queue_signature_delivery_request(delivery_row.id);
  exception when others then
    blocked := true;
  end;
  insert into signature_delivery_foundation_results values ('Manufacturer queue denied', blocked, 'blocked: ' || blocked);

  perform set_config('request.jwt.claim.sub', admin_uuid::text, true);
  select count(*) into visible_count from public.signature_delivery_requests where id = delivery_row.id;
  insert into signature_delivery_foundation_results values ('Admin read', visible_count = 1, 'visible: ' || visible_count);
  ...
  insert into signature_delivery_foundation_results values ('Admin queue denied', blocked, 'blocked: ' || blocked);

  perform set_config('request.jwt.claim.sub', buyer_uuid::text, true);
  select * into queued_row from public.queue_signature_delivery_request(delivery_row.id);
  insert into signature_delivery_foundation_results values ('Buyer queues draft internally', queued_row.status = 'queued', queued_row.status);
  insert into signature_delivery_foundation_results values ('queued_at database-generated', queued_row.queued_at is not null, coalesce(queued_row.queued_at::text, 'null'));
  insert into signature_delivery_foundation_results values ('queued has no sent state', queued_row.status <> 'sent' and queued_row.request_payload_snapshot->>'provider_contacted' = 'false' and queued_row.request_payload_snapshot->>'email_sent' = 'false' and queued_row.request_payload_snapshot->>'signing_link_created' = 'false', 'queued internal only');
  ...
  insert into signature_delivery_foundation_results values ('second queue denied', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    perform public.cancel_signature_delivery_request(delivery_row.id, ' ');
  exception when others then
    blocked := true;
  end;
  insert into signature_delivery_foundation_results values ('blank cancel reason denied', blocked, 'blocked: ' || blocked);

  select * into cancelled_row from public.cancel_signature_delivery_request(delivery_row.id, 'Rollback verification cancellation');
  ...
  insert into signature_delivery_foundation_results values ('cancelled request terminal', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    update public.signature_delivery_requests set status = 'delivery_draft' where id = delivery_row.id;
  exception when others then
    blocked := true;
  end;
  insert into signature_delivery_foundation_results values ('direct request update denied', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    delete from public.signature_delivery_requests where id = delivery_row.id;
  exception when others then
    blocked := true;
  end;
  insert into signature_delivery_foundation_results values ('direct request delete denied', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    update public.signature_delivery_recipients set delivery_status = 'sent' where delivery_request_id = delivery_row.id;
  exception when others then
    blocked := true;
  end;
  insert into signature_delivery_foundation_results values ('direct recipient update denied', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    insert into public.signature_delivery_events(delivery_request_id, event_type, actor_profile_id, metadata)
    values (delivery_row.id, 'signature_delivery_queued', buyer_uuid, '{"actor_id":"bad","provider_token":"bad"}'::jsonb);
  exception when others then
    blocked := true;
  end;
  insert into signature_delivery_foundation_results values ('direct event forgery denied', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    update public.signature_delivery_events set metadata = '{"changed":true}'::jsonb where delivery_request_id = delivery_row.id;
  exception when others then
    blocked := true;
  end;
  insert into signature_delivery_foundation_results values ('events immutable', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    delete from public.signature_delivery_events where delivery_request_id = delivery_row.id;
  exception when others then
    blocked := true;
  end;
  insert into signature_delivery_foundation_results values ('events undeletable', blocked, 'blocked: ' || blocked);
  ...
  insert into signature_delivery_foundation_results values (
    ...
    not exists (
      ...
      where delivery_request_id = delivery_row.id
        and (metadata ? 'actor_id' or metadata ? 'provider_token' or metadata ? 'signing_url')
    ),
    'metadata checked'
  );

  perform set_config('request.jwt.claim.sub', buyer_uuid::text, true);
  select count(*) into visible_count from public.signature_delivery_recipients where delivery_request_id = delivery_row.id;
  insert into signature_delivery_foundation_results values ('Buyer recipient read', visible_count = 2, 'visible: ' || visible_count);
  ...
  insert into signature_delivery_foundation_results values ('Anonymous denied', blocked or visible_count = 0, 'blocked: ' || blocked || ', visible: ' || visible_count);
  reset role;
end;
$$;
...
select jsonb_pretty(jsonb_build_object(
  ...
  (
    ...
    from signature_delivery_foundation_results
  )
)) as signature_delivery_foundation_security_results;
...
begin
  ...
  if failed_checks is not null then
    raise exception 'Signature delivery foundation security checks failed: %', failed_checks;
  end if;
end;
$$;
