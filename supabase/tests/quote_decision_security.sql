...
create temp table quote_decision_security_results (
  check_name text primary key,
  passed boolean not null,
  detail text not null
) on commit drop;

create temp table quote_decision_subjects (
  subject_name text primary key,
  subject_id uuid not null
) on commit drop;

grant select, insert on quote_decision_security_results to anon, authenticated;
grant select, insert on quote_decision_subjects to anon, authenticated;
...
begin
  ...
  values
    ('00000000-0000-0000-0000-000000000000', buyer_id, 'authenticated', 'authenticated', 'decision-buyer-' || buyer_id || '@example.test', 'placeholder', now(), '{"provider":"email","providers":["email"]}'::jsonb, '{"full_name":"Decision Buyer","role":"buyer"}'::jsonb, now(), now(), false, false),
    ('00000000-0000-0000-0000-000000000000', other_buyer_id, 'authenticated', 'authenticated', 'decision-other-buyer-' || other_buyer_id || '@example.test', 'placeholder', now(), '{"provider":"email","providers":["email"]}'::jsonb, '{"full_name":"Decision Other Buyer","role":"buyer"}'::jsonb, now(), now(), false, false),
    ('00000000-0000-0000-0000-000000000000', manufacturer_owner_id, 'authenticated', 'authenticated', 'decision-manufacturer-' || manufacturer_owner_id || '@example.test', 'placeholder', now(), '{"provider":"email","providers":["email"]}'::jsonb, '{"full_name":"Decision Manufacturer","role":"manufacturer"}'::jsonb, now(), now(), false, false),
    ('00000000-0000-0000-0000-000000000000', other_manufacturer_owner_id, 'authenticated', 'authenticated', 'decision-other-manufacturer-' || other_manufacturer_owner_id || '@example.test', 'placeholder', now(), '{"provider":"email","providers":["email"]}'::jsonb, '{"full_name":"Decision Other Manufacturer","role":"manufacturer"}'::jsonb, now(), now(), false, false),
    ('00000000-0000-0000-0000-000000000000', admin_id, 'authenticated', 'authenticated', 'decision-admin-' || admin_id || '@example.test', 'placeholder', now(), '{"provider":"email","providers":["email"]}'::jsonb, '{"full_name":"Decision Admin","role":"buyer"}'::jsonb, now(), now(), false, false);

  update public.profiles set role = 'admin' where id = admin_id;

  insert into quote_decision_subjects(subject_name, subject_id)
  values
    ('buyer', buyer_id),
...
declare
  ...
  product_id uuid;
  accept_rfq_id uuid;
  accept_quote_id uuid;
  reject_rfq_id uuid;
  reject_quote_id uuid;
  revision_rfq_id uuid;
  revision_quote_id uuid;
  revision_draft_id uuid;
  revision_submitted_id uuid;
  draft_rfq_id uuid;
  draft_quote_id uuid;
  voluntary_rfq_id uuid;
  voluntary_quote_id uuid;
  noncurrent_rfq_id uuid;
  noncurrent_quote_id uuid;
...
begin
  select subject_id into buyer_id from quote_decision_subjects where subject_name = 'buyer';
  select subject_id into other_buyer_id from quote_decision_subjects where subject_name = 'other_buyer';
  select subject_id into manufacturer_owner_id from quote_decision_subjects where subject_name = 'manufacturer_owner';
  select subject_id into other_manufacturer_owner_id from quote_decision_subjects where subject_name = 'other_manufacturer_owner';
  select subject_id into admin_id from quote_decision_subjects where subject_name = 'admin';
  ...
  values (manufacturer_owner_id, 'Decision Factory Legal', 'Decision Factory', 'China', 'draft')
  returning id into manufacturer_id;

  insert into public.manufacturers(owner_id, company_name, company_display_name, country, application_status)
  values (other_manufacturer_owner_id, 'Other Decision Factory Legal', 'Other Decision Factory', 'Vietnam', 'draft')
  ...
  values (manufacturer_id, 'Decision Test Product', 'Modular', 'USD', 'published')
  ...
  values (buyer_id, manufacturer_id, product_id, 'submitted', 1, 'USD', 'Canada')
  returning id into accept_rfq_id;

  insert into public.rfqs(buyer_id, manufacturer_id, product_id, status, requested_quantity, requested_currency, destination_country)
  values (buyer_id, manufacturer_id, product_id, 'submitted', 1, 'USD', 'Canada')
  returning id into reject_rfq_id;

  insert into public.rfqs(buyer_id, manufacturer_id, product_id, status, requested_quantity, requested_currency, destination_country)
  values (buyer_id, manufacturer_id, product_id, 'submitted', 1, 'USD', 'Canada')
  returning id into revision_rfq_id;
  ...
  insert into public.rfqs(buyer_id, manufacturer_id, product_id, status, requested_quantity, requested_currency, destination_country)
  values (buyer_id, manufacturer_id, product_id, 'submitted', 1, 'USD', 'Canada')
  returning id into voluntary_rfq_id;

  insert into public.rfqs(buyer_id, manufacturer_id, product_id, status, requested_quantity, requested_currency, destination_country)
  values (buyer_id, manufacturer_id, product_id, 'submitted', 1, 'USD', 'Canada')
  returning id into noncurrent_rfq_id;

  perform set_config('request.jwt.claim.sub', manufacturer_owner_id::text, true);

  update public.rfqs
  set status = 'manufacturer_review'
  where id in (accept_rfq_id, reject_rfq_id, revision_rfq_id, draft_rfq_id, voluntary_rfq_id, noncurrent_rfq_id);

  select id into accept_quote_id from public.create_rfq_quote_draft(accept_rfq_id);
  insert into public.rfq_quote_items(quote_id, line_order, item_type, description, quantity, unit_price)
  values (accept_quote_id, 1, 'product', 'Accept unit', 1, 100000);
  select id into accept_quote_id from public.submit_rfq_quote(accept_quote_id);

  select id into reject_quote_id from public.create_rfq_quote_draft(reject_rfq_id);
  insert into public.rfq_quote_items(quote_id, line_order, item_type, description, quantity, unit_price)
  values (reject_quote_id, 1, 'product', 'Reject unit', 1, 100000);
  select id into reject_quote_id from public.submit_rfq_quote(reject_quote_id);

  select id into revision_quote_id from public.create_rfq_quote_draft(revision_rfq_id);
  insert into public.rfq_quote_items(quote_id, line_order, item_type, description, quantity, unit_price)
  values (revision_quote_id, 1, 'product', 'Revision unit', 1, 100000);
  ...
  select id into voluntary_quote_id from public.create_rfq_quote_draft(voluntary_rfq_id);
  insert into public.rfq_quote_items(quote_id, line_order, item_type, description, quantity, unit_price)
  values (voluntary_quote_id, 1, 'product', 'Voluntary unit', 1, 100000);
  select id into voluntary_quote_id from public.submit_rfq_quote(voluntary_quote_id);

  select id into noncurrent_quote_id from public.create_rfq_quote_draft(noncurrent_rfq_id);
  insert into public.rfq_quote_items(quote_id, line_order, item_type, description, quantity, unit_price)
  values (noncurrent_quote_id, 1, 'product', 'Noncurrent unit', 1, 100000);
  select id into noncurrent_quote_id from public.submit_rfq_quote(noncurrent_quote_id);
  ...
  insert into quote_decision_security_results values (
    ...
    exists (
      select 1 from public.rfq_quote_decisions
      where id = decision_id and quote_id = accept_quote_id and decision = 'accepted' and reason = 'approved'
    ),
    'decision checked'
  );
  insert into quote_decision_security_results values ('accepted quote immutable status', exists (select 1 from public.rfq_quotes where id = accept_quote_id and status = 'accepted'), 'quote status checked');
  insert into quote_decision_security_results values ('RFQ accepted transition', exists (select 1 from public.rfqs where id = accept_rfq_id and status = 'accepted'), 'rfq status checked');
  insert into quote_decision_security_results values ('trusted quote_accepted event', exists (select 1 from public.rfq_events where rfq_id = accept_rfq_id and event_type = 'quote_accepted' and actor_profile_id = buyer_id and metadata->>'quote_id' = accept_quote_id::text and metadata->>'decision_id' = decision_id::text), 'event checked');

  select d.buyer_id, d.rfq_id into stored_uuid, stored_text
  from public.rfq_quote_decisions d
  where d.id = decision_id;
  insert into quote_decision_security_results values ('buyer_id and rfq_id database-derived', stored_uuid = buyer_id and stored_text::uuid = accept_rfq_id, 'buyer/rfq checked');

  select id into decision_id from public.reject_rfq_quote(reject_quote_id, 'Not aligned');
  insert into quote_decision_security_results values ('buyer rejects own current submitted quote', exists (select 1 from public.rfq_quote_decisions where id = decision_id and decision = 'rejected'), 'decision checked');
  ...
  insert into quote_decision_security_results values ('revision_requested Quote immutable status', exists (select 1 from public.rfq_quotes where id = revision_quote_id and status = 'revision_requested'), 'quote status checked');
  insert into quote_decision_security_results values ('RFQ revision_requested transition', exists (select 1 from public.rfqs where id = revision_rfq_id and status = 'revision_requested'), 'rfq status checked');
  insert into quote_decision_security_results values ('trusted quote_revision_requested event', exists (select 1 from public.rfq_events where rfq_id = revision_rfq_id and event_type = 'quote_revision_requested' and actor_profile_id = buyer_id), 'event checked');

  blocked := false;
  begin
    perform public.request_rfq_quote_revision(voluntary_quote_id, '');
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('revision request reason required', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    perform public.reject_rfq_quote(voluntary_quote_id, repeat('x', 4001));
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('reason length enforced', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    perform public.accept_rfq_quote(draft_quote_id, null);
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('draft Quote decision denied', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    perform public.accept_rfq_quote(superseded_quote_id, null);
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('superseded Quote decision denied', blocked, 'blocked: ' || blocked);

  perform public.accept_rfq_quote(voluntary_quote_id, null);
  blocked := false;
  begin
    perform public.reject_rfq_quote(voluntary_quote_id, null);
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('duplicate decision denied', blocked, 'blocked: ' || blocked);

  perform set_config('request.jwt.claim.sub', other_buyer_id::text, true);
  blocked := false;
  begin
    perform public.accept_rfq_quote(revision_quote_id, null);
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('Other Buyer denied', blocked, 'blocked: ' || blocked);
  ...
  insert into quote_decision_security_results values ('Manufacturer decision denied', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    insert into public.rfq_events(rfq_id, event_type, actor_profile_id, metadata)
    values (revision_rfq_id, 'quote_revision_requested', manufacturer_owner_id, '{"actor_profile_id":"fake"}'::jsonb);
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('direct event forgery denied', blocked, 'blocked: ' || blocked);

  select id into revision_draft_id from public.create_rfq_quote_revision(revision_quote_id);
  insert into quote_decision_security_results values ('revision draft allowed only after revision request', exists (select 1 from public.rfq_quotes where id = revision_draft_id and status = 'draft' and version = 2), 'revision draft checked');

  blocked := false;
  begin
    perform public.create_rfq_quote_revision(voluntary_quote_id);
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('voluntary revision from current submitted Quote denied', blocked, 'blocked: ' || blocked);

  select id into revision_submitted_id from public.submit_rfq_quote(revision_draft_id);
  insert into quote_decision_security_results values ('revision submission moves RFQ revision_requested to quoted', exists (select 1 from public.rfqs where id = revision_rfq_id and status = 'quoted'), 'rfq status checked');
  ...
  insert into quote_decision_security_results values ('accepted Quote immutable', blocked or exists (select 1 from public.rfq_quotes where id = accept_quote_id and origin_port is distinct from 'Changed'), 'blocked: ' || blocked);

  blocked := false;
  begin
    update public.rfq_quote_decisions set reason = 'changed' where id = decision_id;
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('decision rows immutable', blocked or not exists (select 1 from public.rfq_quote_decisions where id = decision_id and reason = 'changed'), 'blocked: ' || blocked);

  blocked := false;
  begin
    delete from public.rfq_quote_decisions where id = decision_id;
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('decision rows undeletable', blocked or exists (select 1 from public.rfq_quote_decisions where id = decision_id), 'blocked: ' || blocked);

  perform set_config('request.jwt.claim.sub', admin_id::text, true);
  blocked := false;
  begin
    perform public.reject_rfq_quote(revision_submitted_id, null);
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('Admin direct decision denied', blocked, 'blocked: ' || blocked);

  blocked := false;
  begin
    update public.rfqs set status = 'accepted' where id = revision_rfq_id;
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('Admin ordinary lifecycle bypass denied', blocked, 'blocked: ' || blocked);

  perform set_config('request.jwt.claim.sub', buyer_id::text, true);
  select count(*) into visible_count from public.rfq_quote_decisions where quote_id in (accept_quote_id, reject_quote_id, revision_quote_id);
  insert into quote_decision_security_results values ('Buyer read isolation own decisions', visible_count = 3, 'visible: ' || visible_count);

  perform set_config('request.jwt.claim.sub', other_buyer_id::text, true);
  select count(*) into visible_count from public.rfq_quote_decisions where quote_id in (accept_quote_id, reject_quote_id, revision_quote_id);
  insert into quote_decision_security_results values ('other Buyer denied', visible_count = 0, 'visible: ' || visible_count);

  perform set_config('request.jwt.claim.sub', manufacturer_owner_id::text, true);
  select count(*) into visible_count from public.rfq_quote_decisions where quote_id in (accept_quote_id, reject_quote_id, revision_quote_id);
  insert into quote_decision_security_results values ('Manufacturer assigned read', visible_count = 3, 'visible: ' || visible_count);

  perform set_config('request.jwt.claim.sub', other_manufacturer_owner_id::text, true);
  select count(*) into visible_count from public.rfq_quote_decisions where quote_id in (accept_quote_id, reject_quote_id, revision_quote_id);
  insert into quote_decision_security_results values ('other Manufacturer denied', visible_count = 0, 'visible: ' || visible_count);

  perform set_config('request.jwt.claim.sub', admin_id::text, true);
  select count(*) into visible_count from public.rfq_quote_decisions where quote_id in (accept_quote_id, reject_quote_id, revision_quote_id);
  insert into quote_decision_security_results values ('Admin read', visible_count = 3, 'visible: ' || visible_count);

  blocked := false;
  begin
    perform public.decide_rfq_quote(revision_submitted_id, 'accepted', null);
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('internal decision helper invocation denied', blocked, 'blocked: ' || blocked);
end;
$$;
...
begin
  begin
    perform public.accept_rfq_quote(gen_random_uuid(), null);
  exception when others then
    blocked := true;
  end;
  ...
  blocked := false;
  begin
    select count(*) into visible_count from public.rfq_quote_decisions;
  exception when others then
    blocked := true;
  end;
  insert into quote_decision_security_results values ('Anonymous cannot read decisions', blocked or visible_count = 0, 'blocked: ' || blocked || ', visible: ' || visible_count);
end;
$$;

select check_name, passed, detail
from quote_decision_security_results
order by check_name;
